...
- **No explicit request to leave content unchanged** (e.g., no content requirements stated, or requests like "help me enrich this", "there's too little content, add more", "expand the details"): **Must proceed to step3 to supplement text content, supporting arguments, data, case studies, etc.**, and clearly mark which content was supplemented. But be careful to preserve the outline structure.
- When the user **explicitly requests no content changes** (e.g., "follow my outline exactly", "don't add or remove content", "strictly follow the outline"): Skip step3 and **strictly follow the content provided by the user** -- do not add, remove, expand, or rephrase. Only perform necessary page allocation and page type mapping. When cover/ending pages are missing, ask the user whether they want them added rather than adding them on your own.

### step3. Information Supplementation (When No Explicit Request to Leave Content Unchanged)
...
2. Faithful to user outline: Strictly build upon the user's original points cited in the outline, without deviating from the outline direction
